...
namespace EnemyRelevelerPlus
{
    ...
    public class Program
    {
        ...
        static Lazy<EnemyRelevelerPlusSettings> _settings = null!;
        public static EnemyRelevelerPlusSettings PlusSettings => _settings.Value;
        ...
        public static void RunPatch(IPatcherState<ISkyrimMod, ISkyrimModGetter> state)
        {
            ...
            {
                Console.Error.WriteLine($"ERROR: Missing extraSettingsDataPath");
                return;
            }
            var creatureRulesPath = Path.Combine(state.DefaultSettingsDataPath, "enemy_rules.json");
            ...
            foreach (var getter in state.LoadOrder.PriorityOrder.Npc().WinningOverrides())
            {
                ...
                if (NpcsToIgnore.Contains(getter)
                    ...
                    || PlusSettings.ModsToExclude.Contains(getter.FormKey.ModKey))
                {
                    continue;
                }
                ...
                foreach (var rank in getter.Factions)
                {
                    ...
                    if (enemyRules != null && enemyRules.TryGetValue(faction, out var rule))
                    {
                        skip = false;
                        _rule = rule;
                    }
